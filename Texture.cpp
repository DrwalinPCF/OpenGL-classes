...
int Texture::Load(const char* fileName, int paramWrap, int paramFilter, bool generateMipMap) {
	...
	UpdateTextureData(image, width, height, paramWrap, paramFilter, generateMipMap, GL_TEXTURE_2D, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE);
	SOIL_free_image_data(image);
	return 0;
}

void Texture::UpdateTextureData(const void* data, unsigned w, unsigned h, int paramWrap, int paramFilter, bool generateMipMap, GLenum target, GLenum internalformat, GLenum dataformat, GLenum datatype) {
